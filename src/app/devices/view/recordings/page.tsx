...
function RecordingsContent() {
    ...
    const activeRecordingInList = recordings.find(r => r.status === 'recording' || r.status === 'pending' || r.status === 'uploading');

    // IF we have a recording in the list with duration > 0 AND status is uploaded, it means it's FINISHED.
    // We should NOT show it in the 'Active Stream' section as 'recording' or 'uploading'.
    const isActuallyFinished = activeRecordingInList && activeRecordingInList.status === 'uploaded';

    const effectiveStatus: RecordingStatus = (recordingStatus.status !== 'idle')
        ? recordingStatus
        : (activeRecordingInList && !isActuallyFinished)
            ? {
                // Map status from list to UI status
                status: activeRecordingInList.status === 'uploading' ? 'uploading' : 'recording',
    ...
    useEffect(() => {
        ...
        socketRef.current.on('recording:status', (data: RecordingStatus & { deviceId: string, message?: string }) => {
            if (data.deviceId === deviceId) {
                ...
                // Handle warnings (e.g., accessibility disabled)
                if ((data as any).status === 'warning' && data.message) {
                    setRetryingLogs(prev => [...prev.slice(-3), `⚠️ ${data.message}`]);
                    return;
                }
    ...
    useEffect(() => {
        if (deviceId) {
            markSectionViewed(deviceId, 'recordings');
        }
    }, [deviceId]);
    ...
    const downloadRecording = (recording: Recording) => {
        if (recording.status !== 'uploaded') return;
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
        // Use the dedicated download endpoint instead of stream for proper Content-Disposition
        const link = document.createElement('a');
        link.href = `${apiUrl}/api/recordings/download/${recording.id}`;
            ...
            <main className="flex-1 lg:ml-72">
                ...
                <div className="p-4 lg:p-8 max-w-5xl mx-auto animate-fade-in space-y-6">
                    ...
                    <div className="flex flex-col gap-4">
                        <Link href={`/devices/view/?id=${deviceId}`} className="inline-flex items-center gap-2 text-[var(--muted)] hover:text-[var(--foreground)] transition-colors text-sm font-medium w-fit group">
                            <div className="w-8 h-8 rounded-lg bg-[var(--background)] flex items-center justify-center group-hover:bg-[var(--primary-glow)] transition-colors">
                    ...
                    <div className="card bg-amber-50 border-amber-200 p-4 flex items-start gap-3">
                        ...
                        <div className="text-sm text-amber-700">
                            <p className="font-semibold mb-1">Important: Accessibility Service Required</p>
                            <p>Call recording requires the <strong>Accessibility Service</strong> to be enabled on the device. Without it, recordings may be silent. Only the VOICE_RECOGNITION source can capture caller audio, and only when accessibility is ON.</p>
                        </div>
                    </div>
                </div>
            </main>
